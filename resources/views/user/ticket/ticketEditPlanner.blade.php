...
<section class="content-header">
                    <h1>
                        <i class="fa fa-tag"></i> {{trans('ticketEditPlanner.editTicket')}} #{{$ticket->code}}
                        
                    </h1>
                    <ol class="breadcrumb">
                        @include("layout.breadcrumb")
                        <li class="active">{{trans('ticketEditPlanner.editTicket')}} #{{$ticket->code}}</li>
                    </ol>
                </section>
...
{!! Form::model($ticket, array('action' => ['TicketController@updatePlannerTicket', $ticket->code], 'method'=> 'POST', 'class'=>'form-horizontal')) !!}
...
{{trans('ticketEditPlanner.editClient')}}
</label>
<div class="col-md-10">



{!! Form::select('client[]', $clients, $clientSel, array('class'=>'form-control input-sm js-data-client', 'style'=>'width: 100%', 'multiple'=>'multiple', 'data-placeholder'=>trans('ticketEditPlanner.editSelClient'))) !!}
...
{{trans('ticketEditPlanner.editTo')}}
</label>
<div class="col-md-5">
{!! Form::select('targetGroup', $targetGroup, $ticket->target_group_id, array('class'=>'form-control input-sm select2', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticketEditPlanner.editSelGroup'))) !!}
                @if ($errors->has('targetGroup')) <p class="help-block">{{ $errors->first('targetGroup') }}</p> @endif
</div>
<div class="col-md-1 control-label"> {{trans('ticketEditPlanner.or')}} </div>
<div class="col-md-4">

{!! Form::select('targetUsers[]', $targetUser, $targetUserSel, array('class'=>'form-control input-sm select2', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticketEditPlanner.editSelUsers'), 'multiple'=>'multiple')) !!}
...
<div class="control-group" id="for_prio">
    <div class="controls">
        <div class="form-group">
            <label for="" class="col-sm-2 control-label">{{trans('ticketEditPlanner.prio')}}</label>
            <div class="col-sm-10" style=" padding-top: 5px; ">
                ...
                <div class="btn-group btn-group-justified">
                    <div class="btn-group">
                        ...
                        <i id="lprioclass" class="fa fa-check"></i> {{trans('ticketEditPlanner.low')}}</button>
                        @else
                        <button type="button" class="btn prio bg-purple btn-xs active" data-value="low">
                        <i id="lprioclass" class=""></i> {{trans('ticketEditPlanner.low')}}</button>
                        @endif
                        
                    </div>
                    <div class="btn-group">
                     @if ($ticket->prio == "normal") 
                        <button type="button" class="btn prio bg-orange btn-xs active" data-value="normal"><i id="lprioclass" class="fa fa-check"></i> {{trans('ticketEditPlanner.norm')}}</button>
                     @else
                        <button type="button" class="btn prio bg-orange btn-xs" data-value="normal"><i id="lprioclass" class=""></i> {{trans('ticketEditPlanner.norm')}}</button>
                     @endif
                    </div>
                    <div class="btn-group">
                    @if ($ticket->prio == "high") 
                    <button type="button" class="btn prio bg-maroon btn-xs active" data-value="high"><i id="lprioclass" class="fa fa-check"></i> {{trans('ticketEditPlanner.high')}}</button>
                    @else
                        <button type="button" class="btn prio bg-maroon btn-xs" data-value="high"><i id="lprioclass" class=""></i> {{trans('ticketEditPlanner.high')}}</button>
                    @endif
                    </div>
                </div>
            </div></div></div></div>
...
<label for='unit' class="col-md-2 control-label">
{{trans('ticketEditPlanner.subj')}}
</label>
<div class="col-md-5">
{!! Form::select('subj', $subj, $ticket->subject, array('class'=>'form-control input-sm select2', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticketEditPlanner.selSubj'))) !!}
...
{!! Form::select('tags[]', $tags, $tagsSel, array('class'=>'form-control input-sm select2-tags', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticketEditPlanner.tags'), 'multiple'=>'multiple')) !!}
...
<div class="col-md-5">
{!! Form::text('subj', $ticket->subject, array('class'=>'form-control input-sm', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticketEditPlanner.selSubj'))) !!}
                  @if ($errors->has('subj')) <p class="help-block">{{ $errors->first('subj') }}</p> @endif
</div>
<div class="col-md-5">
{!! Form::select('tags[]', $tags, $tagsSel, array('class'=>'form-control input-sm select2-tags', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticketEditPlanner.tags'), 'multiple'=>'multiple')) !!}
...
<div class="col-md-12 file-element">
                    ...
                    <div class="attachment-pushed" style="margin-left: 22px;">
                      <div class="attachment-text">
                        {{$file->name }} <small> 
                        ({!! round(File::size(storage_path('users/'.$file->user_id.'/'.$file->hash.'.'.$file->extension))/1024/1024, 3) !!} {{trans('ticketEditPlanner.Mb')}})</small>

                        <button data-hash="{{$file->hash}}" class="pull-right btn btn-default btn-xs delete_file">{{trans('ticketEditPlanner.delete')}}</button>
...
<div class="dz-clickable" id="myid">
                                ...
                                <h4 class="text-muted">{{trans('ticketEditPlanner.dropFile')}}</h4>
                                <p class="text-muted">
                                    {{trans('ticketEditPlanner.dropOrClick')}}
                                </p>
                                </div>
                                                                            </div>
...
<span>{{trans('ticketEditPlanner.delete')}}</span>
</button></td>
</tr>
</tbody></table>
                                                                            </div>
                      ...
                      <div class="box-header with-border">
                        <h4 class="box-title">
                          <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" class="" aria-expanded="true">
                           <small>  {{trans('ticketEditPlanner.addFields')}} </small>
                          </a>
                        </h4>
                      ...
                      <div class="box-header with-border">
                        <h4 class="box-title">
                          <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne" aria-expanded="false">
                           <small> {{trans('ticketEditPlanner.options')}} </small>
                          </a>
                        </h4>
                  <div class="box-tools pull-right">
...
@if ($TicketForm->slas->count() > 0 )
                    ...
                    {!! Form::label('sla', trans('ticketEditPlanner.slaPlan'), array('class'=>'col-sm-3 control-label')) !!}
                    <div class="col-sm-9">
                    {!! Form::select('sla', $slas, $ticket->sla_id, array('class'=>'form-control input-sm select2', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticketEditPlanner.selSla'))) !!}
...
<label for='unit' class="col-md-3 control-label">
{{trans('ticketEditPlanner.Watching')}}
</label>
<div class="col-md-9">

{!! Form::select('watchingUsers[]', $watchingUsers, $watchingUsersSel, array('class'=>'form-control input-sm select2', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticketEditPlanner.editSelUsers'), 'multiple'=>'multiple')) !!}
...
<div class="col-md-9">
            ...
            <label>
              ...
              {{trans('ticketEditPlanner.OnceSuccess')}}
            </label>
...
<div class="col-md-9">
            ...
            <label>
              {!! Form::checkbox('check_after', 'true', $check_afterStatus, ['class'=>'minimal']); !!} 
               {{trans('ticketEditPlanner.ApproveAfterOk')}}
            </label>
...
@endif
                    ...
                    @include("user.ticket.plannerMenuEdit")
...
<div class="col-md-10" id="processing">
    <div class="btn-group btn-group-justified">
        <div class="btn-group">
            ...
            {!! Form::button(trans('ticketEditPlanner.Save'), array('type' => 'submit', 'class'=>'btn btn-success btn-flat')); !!}

        </div>
        <div class="btn-group">
            <button href="{{URL::to('/ticket/create')}}" id="del_ticket" class="btn btn-danger" ><i class="fa fa-eraser"></i> {{trans('ticketEditPlanner.DeleteTicket')}}</button>
        </div>
    </div>
...
@if ($ticket->planners->log->count() > 0 )
<div class="box box-default">
                    <div class="box-header with-border">
                      <h3 class="box-title">{{trans('ticketEditPlanner.LogActions')}}</h3>
...
<table class="table table-striped">
                ...
                <tr>
                  ...
                  <th>{{trans('ticketEditPlanner.Date')}}</th>
                </tr>
@foreach($ticket->planners->log as $log)
                <tr>
                  <td><a href="{{URL::to('/ticket/'.$log->ticket->code)}}">{{$log->ticket->code}}</a></td>
                  <td>{{LocalizedCarbon::instance($log->ticket->created_at)->formatLocalized('%d %f %Y, %H:%M')}}</td>
                </tr>
@endforeach
...
$.ajax({
                    type: "POST",
                    url: "{{URL::to('/ticket/planner/delete/'.$ticket->code) }}",
                    //dataType: "json",
                    data: {
                      _token : CSRF_TOKEN,
                      _method: 'DELETE'
                    ...
                    success: function(html) {
                      ...
                      window.location = "{{URL::to('/ticket/planner')}}";
