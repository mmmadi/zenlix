...
<section class="content-header">
                    <h1>
                        <i class="fa fa-ticket"></i> {{trans('ticket.ticket')}} <strong>#{{$ticket->code}}</strong>
                        <small>
                    ...
                    <ol class="breadcrumb">
                        ...
                        <li class="active">{{trans('ticket.ticket')}} #{{$ticket->code}}</li>
                    </ol>
                </section>
...
<div class="box box-widget">
                <div class="box-header with-border">
                  ...
                  <div class="user-block">
                    ...
                    <span class="description">{{trans('ticket.targetTo')}}
...
@endif
                    ...
                    @include("user.ticket.ticketDeadline")
                    @include("user.ticket.ticketTags")
                    </span>
...
<div class="col-md-12">
                    ...
                    <div class="attachment-pushed" style="margin-left: 22px;">
                      <div class="attachment-text">
                        {{$file->name }} <small> 
                        ({!! round(File::size(storage_path('users/'.$file->user_id.'/'.$file->hash.'.'.$file->extension))/1024/1024, 3) !!} {{trans('ticket.Mb')}})</small>
...
@if ($ticket->merged->count() > 0 )
<div class="col-md-12">

<h4>{{trans('ticket.linked')}}</h4>
<table class="table table-bordered">
<tbody>
@foreach ($ticket->merged as $merge)

  <tr>
<td style="width: 100px;"><a href="{{URL::to('/ticket/').'/'.$merge->code}}"> #{{$merge->code}}</a></td>
<td>{{$merge->subject}}</td>
  </tr>
@endforeach
</tbody>
...
<button rel="tooltip" data-placement="bottom" title="{{trans('ticket.btnDel')}}" id="del_ticket" type="button" class="btn btn-default btn-xs"><i class="fa fa-trash"></i> </button>

<a rel="tooltip" data-placement="bottom" title="{{trans('ticket.btnEdit')}}" href="{{URL::to('/ticket/edit/'.$ticket->code)}}" class="btn btn-default btn-xs"><i class="fa fa-pencil"></i>  </a>

<a rel="tooltip" data-toggle="modal" data-placement="bottom" title="{{trans('ticket.btnLink')}}" id="merge_ticket" type="button" class="btn btn-default btn-xs" data-target="#myModalMerge"><i class="fa fa-code-fork"></i> </a>
...
<a rel="tooltip" data-placement="bottom" title="{{trans('ticket.btnPrint')}}" href="{{URL::to('/ticket/'.$ticket->code.'/print')}}" class="btn btn-default btn-xs"><i class="fa fa-print"></i> </a>
                  ...
                  <span class="pull-right text-muted"> @if($ticket->comments->count() > 0) {{$ticket->comments->count()}} {{trans('ticket.comments2')}} @endif</span>
                </div><!-- /.box-body -->
...
<div class="callout callout-info">
                <h4>{{trans('ticket.btnLockInfo')}}</h4>

                <p>{{trans('ticket.btnLockInfoFull')}}</p>
</div>
...
@include("user.ticket.ticketWaitOk")
@endif
...
<div class="nav-tabs-custom">
                                <ul class="nav nav-tabs">
                                    <li class="active"><a href="#tab_1" data-toggle="tab"><i class="fa fa-comments-o"></i> {{trans('ticket.barComments')}}</a></li>
                                    <li class=""><a href="#tab_2" data-toggle="tab" id="get_new_log">{{trans('ticket.barLogs')}}</a></li>
...
<div class="alert alert-default alert-dismissible" id="noComments">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                <h4><i class="icon fa fa-info"></i> {{trans('ticket.nocomments')}}</h4>
                {{trans('ticket.nocommentsFull')}}
              </div>
@else
...
<form action="#" method="post">
                    ...
                    <div class="img-push"> 
                      ...
                      <textarea id="msg" name="msg" class="form-control trumbowyg" data-toggle="popover" data-html="true" data-trigger="manual" data-placement="top" data-content="<small>{{trans('ticket.pleaseWrite')}}</small>" placeholder="Комментарий к заявке" style="overflow: hidden; word-wrap: break-word; resize: vertical;"></textarea> 


                      <div class="text-muted well well-sm no-shadow dz-clickable" id="myid" style="margin-bottom: 0px;">

                          <div class="dz-message" data-dz-message="">
                            <center class="text-muted"><small>{{trans('ticket.pleaseDrop')}}</small></center>
                          </div>
...
<div class="pull-left">
                ...
                <label>
                  <input id="visible_client" value="true" type="checkbox" class="minimal">
                  {{trans('ticket.notifyClient')}}
                </label>
</div>
<div class="btn-group pull-right">
                                            <button id="do_comment" class="btn btn-success btn-sm "><i class="fa fa-comment"></i> {{trans('ticket.send')}}</button>
    ...
    <div class="box box-widget widget-user-2">
                ...
                <div class="box-footer no-padding">
                  ...
                  @if ($client->profile->email)
                    <li><a>{{trans('ticket.email')}} <span class="pull-right">{{$client->profile->email}}</span></a></li> @endif
                  @if ($client->profile->skype)
                    <li><a>{{trans('ticket.skype')}} <span class="pull-right">{{$client->profile->skype}}</span></a></li>@endif
                  @if ($client->profile->telephone)
                    <li><a>{{trans('ticket.tel')}} <span class="pull-right">{{$client->profile->telephone}}</span></a></li>@endif
                  @if ($client->profile->address)
                    <li><a>{{trans('ticket.address')}} <span class="pull-right">{{$client->profile->address}}</span></a></li>@endif
                  </ul>
                </div>
    </div>
@endforeach
...
<div class="box box-default">
                    <div class="box-header with-border">
                      <h3 class="box-title">{{trans('ticket.watching')}}</h3>
...
@if ($AccessAction == true)
                    <div class="box-footer text-center">
                      <a href="" class="uppercase" data-toggle="modal" data-target="#myModal">{{trans('ticket.EditWatch')}}
                    </a>
                    </div><!-- /.box-footer -->
@endif
...
<!-- Modal -->
<div class="modal fade" id="myModalMerge" role="dialog" aria-labelledby="myModalLabelMerge">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabelMerge">{{trans('ticket.btnLink')}}</h4>
      </div>
...
{!! Form::select('MergeTicketList', [], Null, array('class'=>'form-control input-sm js-data-ticket-merge', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticket.codeTicket'))) !!}
...
<hr style="margin-bottom: 0px;">
        <ul class="products-list product-list-in-box" id="merge-data">


@include('user.ticket.mergeTicketList')
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">{{trans('ticket.close')}}</button>
...
<div class="modal fade" id="myModal" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">{{trans('ticket.userWatching')}}</h4>
      </div>
      <div class="modal-body">
...
{!! Form::select('watchingUserList', [], Null, array('class'=>'form-control input-sm js-data-watching-client', 'style'=>'width: 100%', 'data-placeholder'=>trans('ticket.selUsers'))) !!}
...
<hr style="margin-bottom: 0px;">
        <ul class="products-list product-list-in-box" id="watching-data">


@include('user.ticket.watchingList')
...
var MOMENTJS_DAY ='{{trans('ticket.shortDay')}}',
    MOMENTJS_HOUR ='{{trans('ticket.shortHour')}}',
    MOMENTJS_MINUTE='{{trans('ticket.shortMin')}}',
    MOMENTJS_SEC='{{trans('ticket.shortSec')}}';
...
ids = [];
    ...
    function formatRepoMerged (repo) {
      if (repo.loading) return repo.text;

      var markup = "<div class='select2-result-repository clearfix'>" +
        "<div class='select2-result-repository__avatar'>" + repo.img + "</div>" +
        "<div class='select2-result-repository__meta'>" +
          "<div class='select2-result-repository__title'>" + repo.name + "</div>";
...
function initMerge() {
$(".js-data-ticket-merge").select2({
  allowClear: true,
  ajax: {
    url: "{{ URL::to('/ticket/merged') }}",
    dataType: 'json',
    allowClear: true,
    placeholder: "Select an attribute",
    delay: 250,
  ...
  escapeMarkup: function (markup) { return markup; }, // let our custom formatter work
  minimumInputLength: 1,
  templateResult: formatRepoMerged, // omitted for brevity, see the source of this page
  templateSelection: formatRepoSelection // omitted for brevity, see the source of this page
});
}
initMerge();
...
$(".js-data-ticket-merge").on("change", function (e) {


console.log($(this).val());

var TicketList=$(this).val();

$.ajax({
                    type: "POST",
                    url: "{{URL::to('/ticket/'.$ticket->code.'/merge') }}",
                    //dataType: "json",
                    data: {
                      _token : CSRF_TOKEN,
                      _method: 'PATCH',
                    ...
                    success: function(html) {
                      ...
                      //console.log('ok');
                        ...
                        $(".js-data-ticket-merge").val(null).select2("destroy");
                        initMerge();

                        $.ajax({
                              type: "GET",
                              url: "{{URL::to('/ticket/merged/view') }}",
                              //dataType: "json",
                              data: {
                                _token : CSRF_TOKEN,
                                //_method: 'PATCH',
                              ...
                              success: function(html) {
                                    $("#merge-data").hide().html(html).fadeIn(500);
                              }
                        });
...
//removeMerge
$('body').on('click', '.removeMerge', function(event) {
            event.preventDefault();

            var TicketList=$(this).attr('data-id');
            $.ajax({
                    type: "POST",
                    url: "{{URL::to('/ticket/'.$ticket->code.'/merge') }}",
                    //dataType: "json",
