...
<div class="box ">
<div class="box-body">
 {!! Form::open(array('action' => array('GroupsController@storePost', $group->group_urlhash), 'method'=> 'POST', 'class'=>'form-horizontal')) !!}

                     <img class="img-responsive img-circle img-sm" src="{{$userImgSmall}}" alt="alt text">
                     <!-- .img-push is used to add margin to elements next to floating images -->
                     <div class="img-push @if ($errors->has('text')) has-error @endif">


  {!! Form::textarea('text', null, array('class'=>'form-control input-sm', 'rows'=>'1', 'placeholder'=>trans('group.fillBrain'))) !!}
 ...
 <div class="pull-left" style="padding-left: 40px;">
     ...
     font-weight: 500;
     font-size: 12px;">
    {!! Form::checkbox('mark', 'true', false, array('class' => 'minimal')); !!}

                   {{trans('group.warningPost')}}
                 </label>
                 <label style="
     ...
     font-size: 12px;
 ">
                   {!! Form::checkbox('comment_status', 'true', true, array('class' => 'minimal')); !!}
                   {{trans('group.allowComments')}}
                 </label>
 </div>
 ...
 {!! HTML::decode(Form::button('<i class=\'fa fa-comment\'></i> '.trans('group.save'), array('type' => 'submit', 'class'=>'btn bg-purple btn-xs btn-flat'))) !!}
...
<div class="alert alert-info alert-dismissible">
                ...
                <h4><i class="icon fa fa-info"></i> {{trans('group.empty')}}</h4>
                {{trans('group.groupEmptyMsg')}}
              </div>
...
<a href="{{URL::to('/group/'.$group->group_urlhash.'/post/'.$feed->feed_urlhash)}}"> <small> {{trans('group.more')}} {!! $feed->comments()->count()-2 !!} {{trans('group.comments')}} </small>
</a>
                  </div><!-- /.box-comment -->
@endif


@foreach ($feed->commentsShort->reverse() as $comment)
...
@if ($CurUser->GroupAdminSet($group->id)->count() != 0)
...
<div class="box-footer">
И ещё {{$group->users()->count() - 6 }} {{trans('group.users')}}
</div>
@endif
</div>
...
@if ( ($CurUser->roles->role == 'admin') || ($CurUser->GroupAdminSet($group->id)->count() != 0))
